...
|---|---|---|
| `gateway` | 8080 | Single entry point — JWT validation, rate limiting, routing |
| `auth-service` | 8081 | Registration, login, JWT issuance/refresh, roles |
| `account-service` | 8082 | Account lifecycle, balance management, optimistic locking |
| `transaction-service` | 8083 | Transfer saga orchestration, idempotency, business rules |
| `ledger-service` | 8084 | Immutable double-entry ledger, reconciliation |
| `notification-service` | 8085 | Email/in-app notifications via Kafka events |
| `fraud-service` | 8086 | Async fraud scoring (rules → ML), admin alerts |
| `ai-assistant-service` | 8087 | LLM spending assistant, transaction categorisation |
...
- [ ] **Phase 2 — Core Money Path:** Account Service, Transaction Service (idempotency + optimistic locking)
...
- **Idempotency via DB constraint** — The `UNIQUE(initiated_by, idempotency_key)` DB constraint is the source of truth. Redis provides a fast pre-check but the database wins on conflict.